...
class DatasetCatalog:
    ...
    def get(name):
        if "voc" in name:
            ...
            siemens_root = DatasetCatalog.DATA_DIR
            if 'SIEMENS_ROOT' in os.environ:
                siemens_root = os.environ['SIEMENS_ROOT']

            attrs = DatasetCatalog.DATASETS[name]
            args = dict(
                data_dir=os.path.join(siemens_root, attrs["data_dir"]),
                split=attrs["split"],
            )
            return dict(factory="SIEMENSDataset", args=args)
